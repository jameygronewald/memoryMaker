...
router.post("/", async (req, res) => {
  ...
  const category = await db.Category.create({ categoryName }).catch(errorHandler);
  const event = await db.Event.create(eventData).catch(errorHandler);

  console.log(event)

  const uploadFilesRequests = formData.files['file[]'].map((file) => new Promise((resolve, reject) => {
    ...
    cloudinary.uploader.upload(
      ...
      async (err, image) => {
        ...
        await db.Image.create({ evantId: event.id, name: originalFilename, url: image.url  });
        resolve(image);
      }
    )
  }));
